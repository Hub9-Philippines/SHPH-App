...
export class ChatService {
    static async listThreads(userId, page) {
        if (!userId) {
    ...
    static async getThreadDetails(userId, id) {
        if (!userId) {
            throw new Error('Missing user id');
        }
        const repo = AppDataSource.getRepository(ChatRoom);
    ...
    static async listMessages(userId, threadId, page) {
        if (!userId) {
            throw new Error('Missing user id');
        }
    ...
    static async sendMessage(userId, threadId, content) {
        if (!userId) {
            throw new Error('Missing user id');
        }
        const roomRepo = AppDataSource.getRepository(ChatRoom);
    ...
    static async markRead(userId, threadId) {
        if (!userId) {
            throw new Error('Missing user id');
        }
        const repo = AppDataSource.getRepository(ChatRoom);
    ...
    static async getOrCreateThreadForBooking(userId, bookingId) {
        if (!userId) {
            throw new Error('Missing user id');
        }
        const repo = AppDataSource.getRepository(ChatRoom);
