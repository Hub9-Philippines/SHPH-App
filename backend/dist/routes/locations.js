...
locationsRouter.get('/regions/', async (_req, res) => proxy(res, '/regions.json'));
locationsRouter.get('/provinces/', async (req, res) => {
    const region = req.query.region ? codeSchema.parse(req.query.region) : null;
    return proxy(res, region ? `/regions/${region}/provinces.json` : '/provinces.json');
});
