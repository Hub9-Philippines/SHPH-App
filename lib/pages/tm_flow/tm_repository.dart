...
class PersistentMockTMRepository implements TMRepository {
  ...
  @override
  Stream<TMBookingSnapshot?> watchBookingSnapshot(String requestId) {
    // The SHPH backend does not provide realtime booking streams, so poll
    // the booking endpoint periodically.
    TMBookingSnapshot? _toSnapshot(BookingsRow? booking) {
      if (booking == null) {
        return null;
      }
      final metadata = _extractMetadata(booking.notes);
    ...
    return Stream.periodic(const Duration(seconds: 3))
        .asyncMap((_) async => _toSnapshot(
              await _bookingsService.getBookingById(requestId),
            ));
  }
  ...
  Future<TMProviderProfile?> _findRealProvider({
    ...
    required int attempt,
      ...
      // The SHPH backend does not expose a public provider listing endpoint,
      // so real provider discovery is disabled. The caller will fall back to a
      // synthetic provider.
      null;
