...
class _AvailabilityCalendarWidgetState extends State<AvailabilityCalendarWidget> {
  ...
  Future<void> _loadAvailability() async {
    try {
      ...
      LoggingService.error('Error loading availability: $e', tag: 'AvailCalendar');
      if (!mounted) return;
  ...
  Future<void> _deleteSlot(int id) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to delete slot: $e'), backgroundColor: Colors.red),
      );
    }
  }
  ...
  List<Map<String, dynamic>> get _slotsForSelectedDay {
    ...
    return _availabilitySlots.where((s) {
      final slotDay = s['day_of_week'] ?? s['day']?.toString().toLowerCase() ?? '';
                  ...
                  if (_slotsForSelectedDay.isEmpty)
                    const SliverToBoxAdapter(
                      child: Padding(
                        ...
                        child: Center(child: Text('No slots for this day. Tap + to add one.')),
                      ),
                    )
                  else
                    SliverList(
                      delegate: SliverChildBuilderDelegate(
                        (context, index) {
                          ...
                          final start = slot['start_time']?.toString() ?? '09:00';
                          final end = slot['end_time']?.toString() ?? '17:00';
                          return Card(
                            margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                            child: ListTile(
                              leading: CircleAvatar(
                                backgroundColor: AppTheme.of(context).primary.withOpacity(0.1),
                                child: Icon(Icons.access_time, color: AppTheme.of(context).primary),
                              ),
                              title: Text('$start - $end'),
                              subtitle: Text(slot['note']?.toString() ?? ''),
                              trailing: IconButton(
