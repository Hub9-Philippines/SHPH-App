...
class CallHistoryDetailsPageWidget extends StatefulWidget {
  ...
  @override
  State<CallHistoryDetailsPageWidget> createState() =>
      _CallHistoryDetailsPageWidgetState();
}

class _CallHistoryDetailsPageWidgetState
    extends State<CallHistoryDetailsPageWidget> {
  ...
  Future<void> _updateCall(String action) async {
    final id = widget.callId;
    if (id == null || id.isEmpty) return;
    if (action == 'accept') await ShphChatApi.instance.acceptCall(id);
    if (action == 'reject') await ShphChatApi.instance.rejectCall(id);
    if (action == 'end') await ShphChatApi.instance.endCall(id);
                    ...
                    Padding(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(
                          ...
                          onPressed: () => _updateCall('reject'),
                          icon: const Icon(Icons.call_end),
                          label: const Text('Reject'),
                        )),
                      ]),
                    ),
                  if (widget.callStatus == 'active' ||
                      widget.callStatus == 'connected')
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 24),
                      child: FilledButton.icon(
                        style:
                            FilledButton.styleFrom(backgroundColor: Colors.red),
                        onPressed: () => _updateCall('end'),
                        icon: const Icon(Icons.call_end),
