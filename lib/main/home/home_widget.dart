...
late final List<_ActiveBookingShortcutData> _activeBookingShortcuts;
  ...
  @override
  void initState() {    super.initState();    _model = createModel(context, HomeModel.new);    _activeBookingShortcuts = _buildActiveBookingShortcuts();    _loadAddress();    _loadDeviceLocation();  }  @override  void dispose() {    _mapController?.dispose();    _model.dispose();
...
return;    }        _addressFuture = FFAppState()        .getAddress(      uniqueQueryKey: 'address_$currentUserUid',      requestFn: () async {        final addresses = await AddressesService.instance.getAddresses();        return addresses.map(ApiRowMapper.addressToRow).toList();      },
    )        .catchError((error) {      LoggingService.error(        'Failed to load address: $error',        tag: 'Home',        error: error,      );
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    _buildCategoryChip(
                      ...
                      label: _l10n.hmMore,
                      onTap: () => context.pushNamed(CategoriesWidget.routeName),
                    ),
                  ],
                ),
              ),
...
return BookingFlowController(      initialDraft: BookingDraft(        urgency: BookingUrgency.rightNow,        rooms: 1,        cleaningType: ServiceType.standard,        paymentMethod: BookingPaymentMethod.gcash,        address: BookingAddress(          label: appState.selectedLocationMode == 'device'              ? _l10n.hmCurrentDeviceLocation              : appState.selectedAddressLabel.isNotEmpty                  ? appState.selectedAddressLabel                  : _l10n.bfPinnedLocation,          line1: appState.selectedLocationMode == 'device'              ? _l10n.hmPinnedAddress              : appState.selectedAddressLine1.isNotEmpty                  ? appState.selectedAddressLine1                  : _l10n.hmPinnedAddress,          city: appState.selectedAddressCity.isNotEmpty              ? appState.selectedAddressCity              : 'Metro Manila',        ),        latitude: lat,        longitude: lng,      ),    )..setService(service);  }
...
String _addressText(FFAppState appState) {    if (appState.selectedLocationMode == 'device' && _hasLocation) {      return _l10n.hmCurrentDeviceLocation;
}
...
return _l10n.hmSelectLocation;  }

Future<bool> _showExitConfirmation() async =>      await AppFeedback.confirmDialog(        context: context,                title: _l10n.hmExitApp,
        message: _l10n.hmExitConfirm,
        confirmText: _l10n.hmExit,
        cancelText: _l10n.cancel,      ) ??      false;}
...
return Container(      decoration: const BoxDecoration(        color: Colors.white,        borderRadius: BorderRadius.vertical(top: Radius.circular(28)),      ),      child: SafeArea(        top: false,        child: Padding(          padding: const EdgeInsets.fromLTRB(20, 12, 20, 24),          child: Column(            mainAxisSize: MainAxisSize.min,            crossAxisAlignment: CrossAxisAlignment.start,            children: [              Center(                child: Container(                  width: 42,                  height: 4,                  decoration: BoxDecoration(                    color: const Color(0xFFD6DBE1),                    borderRadius: BorderRadius.circular(999),                  ),                ),              ),              const SizedBox(height: 18),                            Text(
                _l10n.hmChooseLocation,
                style: theme.titleMedium.override(                  font: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700),                  color: const Color(0xFF16202A),                ),              ),              const SizedBox(height: 8),                            Text(
                _l10n.hmChooseLocationSub,
                style: theme.bodySmall.override(                  font: GoogleFonts.plusJakartaSans(),                  color: const Color(0xFF66727E),                ),              ),              const SizedBox(height: 18),              _HomeLocationOption(                                icon: Icons.my_location_rounded,
                title: _l10n.hmCurrentDeviceLocation,
                subtitle: hasDeviceLocation
                    ? _l10n.hmUseLiveLocation
                    : _l10n.hmEnableLocationServices,                isSelected:                    selectedLocationMode == 'device' && hasDeviceLocation,                isEnabled: hasDeviceLocation,                onTap: onUseCurrentLocation,              ),              if (addresses.isNotEmpty) ...[                const SizedBox(height: 18),                Text(                  _l10n.hmSavedAddresses,                  style: theme.labelLarge.override(                    font: GoogleFonts.plusJakartaSans(fontWeight: FontWeight.w700),                    color: const Color(0xFF16202A),                  ),                ),                const SizedBox(height: 10),                ConstrainedBox(                  constraints: const BoxConstraints(maxHeight: 320),                  child: ListView.separated(                    shrinkWrap: true,                    itemCount: addresses.length,                    separatorBuilder: (_, __) => const SizedBox(height: 10),                    itemBuilder: (context, index) {                      final address = addresses[index];
return _HomeLocationOption(                        icon: _addressIcon(address.addressLine2),                        title: address.addressLine2?.trim().isNotEmpty == true                            ? address.addressLine2!.trim()                            : _l10n.hmSavedAddress,                        subtitle: [                          address.addressLine1,                          address.city,                        ]                            .whereType<String>()                            .where((e) => e.trim().isNotEmpty)                            .join(', '),                        isSelected: selectedLocationMode == 'saved' &&                            selectedAddressId == address.id,                        onTap: () => onSelectSavedAddress(address),                      );                    },                  ),                ),              ],              const SizedBox(height: 18),                            SizedBox(
