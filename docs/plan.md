...
|---|---|---|---|
| `AppThemeData.light().primary` | `#368EFF` (blue) | `#63CBD6` (teal) | `lib/theme/app_theme.dart:334` |
| `AppThemeData.dark().primary` | `#368EFF` (blue) | `#63CBD6` (teal) | `lib/theme/app_theme.dart:362` |
| `AppThemeData.light().onPrimary` | `#FFFFFF` | `#0F172A` | `lib/theme/app_theme.dart:335` |
| `AppThemeData.dark().onPrimary` | `#FFFFFF` | `#0F172A` | `lib/theme/app_theme.dart:363` |
| `AppThemeData.light().info` | `#368EFF` (blue) | `#63CBD6` (teal) | `lib/theme/app_theme.dart:351` |
| `AppThemeData.dark().info` | `#63CBD6` (teal) | `#63CBD6` ✅ already | `lib/theme/app_theme.dart:379` |
| `flutter_native_splash.color` | `"#368eff"` (blue) | teal (see A2) | `pubspec.yaml:136` |
| `flutter_native_splash.android_12.color` | `"#368eff"` (blue) | teal (see A2) | `pubspec.yaml:141` |
| `flutter_launcher_icons.adaptive_icon_background` | `'#368eff'` (blue) | teal (see A2) | `pubspec.yaml:130` |
| `android/.../values/colors.xml` `ic_launcher_background` | `#368eff` (blue) | teal (see A2) | `android/app/src/main/res/values/colors.xml` |
| Legacy `flutter_flow_theme.dart` primary | `#63CBD6` (teal) | delete (dead code) | `lib/flutter_flow/flutter_flow_theme.dart` |
...
- [ ] Change `light().primary` `#368EFF` → `#63CBD6` (line 334)
- [ ] Change `dark().primary` `#368EFF` → `#63CBD6` (line 362)
- [ ] Change `light().onPrimary` `#FFFFFF` → `#0F172A` (line 335) — web's `--shph-on-primary`; re-check text contrast on teal fills
- [ ] Change `dark().onPrimary` `#FFFFFF` → `#0F172A` (line 363) — confirm against dark-theme legibility
- [ ] Change `light().info` `#368EFF` → `#63CBD6` (line 351) — kill the light/dark info inconsistency
- [ ] Audit all button/text usages that relied on `onPrimary = white` (e.g. primary CTA text) — teal + `#0F172A` text has 8.3:1 contrast per the web's own token rule
- [ ] Verify the flutter_flow legacy getters at the bottom of `app_theme.dart` (Typography getters) still resolve cleanly against `primaryText` — they must not hardcode blue

**Sweep for stragglers** (blue hex must reach 0 references like the Phase-12 sweep did):

- [ ] `grep -rn "368EFF" lib/ android/ ios/` → replace every UI hit with the matching token (do **not** touch `api`/`services` unless a value is genuinely a UI color)
- [ ] Re-run the Phase-12-style page audit: any page with `Color(0xFF368EFF)` inline → swap to `AppTheme.of(context).primary` / `.info`
...
- [ ] `pubspec.yaml` `flutter_native_splash.color` → `"#0d6d78"`, `android_12.color` → `"#0d6d78"`
- [ ] `pubspec.yaml` `flutter_launcher_icons.adaptive_icon_background` → `'#0d6d78'`
- [ ] `android/app/src/main/res/values/colors.xml` `ic_launcher_background` → `#0d6d78`
- [ ] Regenerate: `dart run flutter_native_splash:create` then `dart run flutter_launcher_icons`
- [ ] Verify `android/app/src/main/res/drawable/launch_background.xml` still points at the right drawables and the generated `splash`/`background` PNGs are the intended color (69-byte placeholder PNGs exist — confirm they render correctly)
- [ ] Android 12+: check `values-v31/styles.xml` `android12splash` drawable and `values-night-*` variants match

**Splash widget** (`lib/pages/splash/splash_widget.dart:90`): already uses `AppTheme.of(context).primary` — automatically correct once A1 lands. No change needed, but verify the logo asset contrast on the new background.

## A3. Delete dead theme code

- [ ] Confirm `flutter_flow_theme.dart` has zero importers (`grep -rln 'flutter_flow_theme' lib/` returned none on 2026-08-09) and remove it — its teal values can confuse future editors
- [ ] Update any docs referencing `flutter_flow_theme.dart`
...
- [ ] `confirmed` → `(0D6D78, D4F0EF)` vs web status token
- [ ] `in_progress` → `(166534, E7F6EC)` vs web
- [ ] `completed` / `cancelled` / `pending` vs web
- [ ] Add missing statuses the web knows (`en_route`, `arrived`, `disputed`) if the Flutter UI surfaces them

## A5. Flutter verification

- [ ] `flutter analyze` → 0 issues
- [ ] `flutter test` → pass
- [ ] `flutter build apk --debug` → splash/launcher visually teal; screenshot light + dark home, settings, booking screens
...
- [ ] **Splash bg:** deep teal `#0D6D78` + white logo (recommended) vs brand teal `#63CBD6` + dark logo (Part A2)
- [ ] **Provider-mode amber in Flutter:** port the web's `body.provider-mode` amber accent, or defer
...
- [ ] Web `variables.css` tokens == Flutter `app_theme.dart` tokens (colors, radii, shadows, statuses)
- [ ] Both apps use Plus Jakarta Sans
- [ ] No `#368EFF` anywhere in Flutter (UI); no blue brand references in web
- [ ] Native splash/launcher (Flutter) match in-app splash
