...
1. Run `C:\Users\Administrator\AppData\Roaming\uv\tools\specify-cli\Scripts\python.exe .specify/scripts/python/check_prerequisites.py --json --paths-only` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   - `FEATURE_DIR`
   - `FEATURE_SPEC`
