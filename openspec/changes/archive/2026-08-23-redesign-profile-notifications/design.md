...
3. **Section groups**: reuse section-header pattern (uppercase muted slate labels) + containers `#F8F9FA` radius 16 holding `TintedMenuTile`s. Row mapping:
   - ACCOUNT: My Bookings → Bookings tab route; Payment & Invoices → PaymentMethods (invoice download rides existing surfaces); Language Preference → LanguageSettings ("English, Hindi, Marathi, etc." copy per brief).
